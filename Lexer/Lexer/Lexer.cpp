...
void SkipComment(size_t &pos, const vector<char> &str) {
	for (; pos < str.size(); pos++) {
		if (pos != 0 && str[pos] == '/' && str[pos - 1] == '*' || (str[pos] == EOF && str[pos] != '€')) {
			break;
		}
	}
}
...
bool tryToMakeString(string &lex, const vector<string> &lexemes, size_t &pos) {
	//„итать от кавычки до кавычки или конца строки
	bool result = false;
	string startLex = lex;
	size_t i = 0;
	string stringLex;
	try {
		...
		pos++;
		stringLex += lex;
		// ≈сли при чтении строки мы дошли до \n, но не дошли до " - вс€ полученна€ строка = ошибка
		for (; i < lexemes.size()
			&& pos < lexemes.size()
			&& lexemes[pos] != EOLN
			&& lexemes[pos] != "\""
